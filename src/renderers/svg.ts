...
export class SVGRenderer {
  ...
  private renderFlowArrows(): string {
    ...
    for (let i = 0; i < segments.length; i++) {
      ...
      elements.push(
        `<polygon points="${tip.x.toFixed(2)},${tip.y.toFixed(2)} ${baseInner.x.toFixed(2)},${baseInner.y.toFixed(2)} ${baseOuter.x.toFixed(2)},${baseOuter.y.toFixed(2)}" fill="${fill}" stroke="${stroke}" stroke-width="${dividerWidth}" stroke-linejoin="round" />`
      );
    }
